    ...
    try:
        ...
        with psycopg2.connect(**config) as conn:
            with conn.cursor() as cur:
                cur.execute(f"SELECT * FROM snake WHERE users=\'{username}\';")
                result = cur.fetchone()
                if result != None:
                    cur.execute(f"SELECT users_score FROM snake WHERE users=\'{username}\';")
                    score = cur.fetchone()
                    return score
                else:
                    cur.execute(f"INSERT INTO snake (users, users_score) VALUES (\'{user_name}\', 0);")
    except (psycopg2.DatabaseError, Exception) as error:
        print(error)

def update(totalscore, username):
    try:
        config = load_config()
        with psycopg2.connect(**config) as conn:
            with conn.cursor() as cur:
                cur.execute(f"UPDATE snake SET users_score={totalscore} WHERE users=\'{username}\';")
    except (psycopg2.DatabaseError, Exception) as error:
        print(error)

user_name = input("Enter your name: ")
user_level = check(user_name)
...
while True:
    ...
    level = font.render(f"{user_level}", True, "white")
    screen.blit(level, (500, 150))
    snake.update()
    apple.update()
